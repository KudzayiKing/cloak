 ...
 *
 * Guard-aware (user feedback round 4): turning Cloaq Mode ON is immediate;
 * turning it OFF asks for PIN / biometric verification when protection is
 * configured in Settings > Cloaq Mode.
 */
...
export function CloakModeToggle({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) {
    return (
      <button
        ...
        aria-label={cloakMode ? "Cloaq Mode is on — verify to turn it off" : "Turn Cloaq Mode on"}
        title={cloakMode ? "Cloaq Mode is on" : "Cloaq Mode is off"}
        className={cn(
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-xl border border-cloak-border bg-cloak-bg/50 p-4">
      <div>
        <p className="text-sm font-medium text-cloak-text">Cloaq Mode</p>
